...
import com.leap.employee.service.JobService;
...
/**
 * REST controller for managing {@link com.leap.employee.domain.Job}.
 */
@RestController
@RequestMapping("/api")
public class JobResource {
     ...
     *         body the new jobDTO, or with status {@code 400 (Bad Request)} if the
     *         job has already an ID.
     ...
     *         or with status {@code 500 (Internal Server Error)} if the jobDTO
     *         couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/jobs/{id}")
    public ResponseEntity<JobDTO> updateJob(@PathVariable(value = "id", required = false) final Long id,
        ...
        return ResponseEntity
                ...
                .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME,
                        jobDTO.getId().toString()))
                .body(result);
    }

    /**
     * {@code PATCH  /jobs/:id} : Partial updates given fields of an existing job,
    ...
    public ResponseEntity<JobDTO> partialUpdateJob(
            ...
            @NotNull @RequestBody JobDTO jobDTO) throws URISyntaxException {
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the jobDTO, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/jobs/{id}")
